...
def processfiles( callsfname, llvmfname, ofilename ):
    global debugactive
    with open(callsfname, 'r') as callsfile,open(ofilename, 'w') as ofile, open("debugfile",'w') as dfile:
        for callstr in callsfile:
            ...
            blockidx = 0
            funcidx = 0
            with open(llvmfname, 'r') as llvmfile:
                for line in llvmfile:
                    ...
                    if block is not None and instmatcher.match(line):
                        ...
                        #        dfile.write(line);
                        if line == callstr:
                            ofile.write(currfunction+"::"+block+"::" + str(funcidx) + "::" + str(blockidx) + "-->"+callstr)
                            found = True
